...
2. **Client-chosen shot quality (critical, newly weaponized).** Strong and Weak are *two separate remotes* — the client picks its own 1000-vs-600 payout tier and can always fire the Strong one. **Fix:** the server verifies stability itself (root-part velocity / `MoveDirection` against the `CameraStats.Stability` thresholds) and **downgrades** Strong→Weak when it disagrees. `CameraSessionTracker` is the natural home for the per-player state.
   **Residual risk (accepted, not eliminated by the fix above):** `MoveDirection`/`AssemblyLinearVelocity`/`HumanoidState` are all properties the client's own character owns and replicates — the downgrade catches an *honest* client that is actually moving, not a *modified* one that reports stillness while moving. Same shape and same accepted posture as the origin risk above (bounded by the live per-subject repeat cooldown, not closed); a real fix needs a server-side displacement sample over a time window, not a single replicated read. See `CaptureGuard.ResolveShotQuality`.
3. **Same-subject farming.** Nothing stops standing still and photographing one target forever; the per-shot `Cooldown` limits rate, not repetition. Anti-farm exists as `CaptureGuard.CheckRepeatPolicy`, driven by each `CaptureTargets.Types[name].RepeatPolicy`. **Corrected (Phase 8.5): both shipped types (`Monster`, `Objective`) now set `RepeatPolicy = "Cooldown"` — this protection is LIVE, an 8-second window per (player, subject), not off.** The rest of this point describes the original ship-it-off decision for context; it is no longer the current state. Re-enabling per-subject farming protection for a future type stays a one-line config flip (`RepeatPolicy = "Cooldown"`), not a re-implementation.
...
9. **Anomaly logging.** `RewardService.Grant` warns above a per-minute XP threshold — cheap, and the first signal that something above is wrong. **Corrected (Phase 8.5): the "retune this because farming is unlimited" premise below no longer holds** — see point 3; repeat farming is cooldown-limited, not unlimited. `ANOMALY_XP_PER_MINUTE` (`RewardService.luau`) should be evaluated against the real cooldown-limited ceiling instead, not left at a value chosen for an unlimited-farming assumption.
